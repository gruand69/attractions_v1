...
from posts.models import Post, Comment, Advice
...
def test_user_can_create_advice(admin_client, advice_form_data, slug_for_country):
    url = reverse('posts:add_advice', args=slug_for_country)
    response = admin_client.post(url, data=advice_form_data)
    assertRedirects(response, reverse('posts:country_town', args=slug_for_country))
...
@pytest.mark.django_db
def test_anomymous_user_cant_create_advice(client, advice_form_data, slug_for_country):
    url = reverse('posts:add_advice', args=slug_for_country)
...
def test_author_can_edit_post(author_client, post_form_data, post, id_for_post):
...
def test_other_user_cant_edit_post(admin_client, post_form_data, post, id_for_post):
    url = reverse('posts:edit_post', args=id_for_post)
...
@pytest.mark.parametrize(
    ...
    ('posts:delete_comment', 
     pytest.lazy_fixture('id_for_post_comment'), 'posts:post_detail', pytest.lazy_fixture('id_for_post'), Comment),
     ('posts:delete_advice', 
     pytest.lazy_fixture('args_for_advice'), 'posts:country_town', pytest.lazy_fixture('slug_for_country'), Advice),
    ('posts:delete_post', 
     pytest.lazy_fixture('id_for_post'), 'posts:index', None, Post)
    ),)
def test_author_can_delete_his_objects(author_client, name, args, redirect_addresses, redirect_args, models):
...
@pytest.mark.parametrize(
    ...
    (
    ('posts:delete_comment', 
     pytest.lazy_fixture('id_for_post_comment'),  Comment),
     ('posts:delete_advice', 
     pytest.lazy_fixture('args_for_advice'), Advice),
    ('posts:delete_post', 
