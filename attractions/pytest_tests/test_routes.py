...
def test_page_availability_for_different_users(parametrized_client, name, args,
                                               expected_status):
    url = reverse(name, args=args)
...
def test_page_change_profile_avalability_for_authorized_user(
        parametrized_client,
        expected_status, username_for_author):
    url = reverse('users:edit_profile', args=username_for_author)
    response = parametrized_client.get(url)
    assert response.status_code == expected_status


def test_page_profile_avalability_for_authorized_user(admin_client,
                                                      username_for_author):
